...
| Structured log entries | `[log(json)@profile.json]{[key(name)@"Mordraga"][sect@address]{[key(city)@"SF"]}}` | `{ "name": "Mordraga", "address": { "city": "SF" } }` | `const profile = { name: "Mordraga", address: { city: "SF" } };` | `jq -n '{name:"Mordraga",address:{city:"SF"}}'` | `serde_json::json!({"name":"Mordraga","address":{"city":"SF"}})` |
