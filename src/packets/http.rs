...
fn allowed_url(cfg: &config::Config, url: &Url) -> bool {
    if cfg.net_allow.is_empty() { return false; }
    let full = url.as_str();
    ...
    for pat in &cfg.net_allow {
        ...
        if p.starts_with("http://") || p.starts_with("https://") {
            if full.starts_with(p) { return true; }
        } else if p.starts_with("*.") {
            let suf = &p[2..];
            if host.ends_with(suf) { return true; }
        } else {
            // bare host
            if host.eq_ignore_ascii_case(p) { return true; }
        }
    }
    false
}
...
pub fn handle(rt: &mut Runtime, p: &Packet) -> Result<Value> {
    if rt.effective_root.is_none() { bail!("E_BOX_REQUIRED: no red.tgsk"); }

    let cfg = config::load(rt.effective_root.as_deref());
    if !cfg.net_enabled { bail!("E_NET_DENY: network disabled by default; enable in .tagspeak.toml [network]"); }

    let method = detect_method(&p.op).ok_or_else(|| anyhow::anyhow!("http needs method: http(get|post|put|delete)"))?;
    ...
    let resp = match resp { Ok(r) => r, Err(e) => bail!(format!("E_HTTP: {e}")) };
...
fn handle_response(mut resp: Response) -> Result<Value> {
    ...
    if ctype.contains("application/json") || ctype.contains("+json") {
        let val: serde_json::Value = resp.json().map_err(|e| anyhow::anyhow!(format!("E_HTTP_JSON:{e}")))?;
