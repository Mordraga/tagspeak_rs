...
pub fn handle(rt: &mut Runtime, p: &Packet) -> Result<Value> {
    let body = p.body.as_ref().ok_or_else(|| anyhow::anyhow!("obj needs body"))?;
    ...
    let doc = Document::new(obj, PathBuf::from(path), String::from("json"), SystemTime::now(), root.clone());
    Ok(Value::Doc(doc))
}
    ...
    for node in body {
        if let Node::Packet(pkt) = node {
            if let Some(k) = parse_key_name(&pkt.op) {
                let v = if let Some(arg) = pkt.arg.as_ref() { arg_to_json(rt, arg)? } else if let Some(inner) = pkt.body.as_ref() { build_object_from_body(rt, inner)? } else { J::Null };
                ...
                let name = if let Some(Arg::Ident(id)) = pkt.arg.as_ref() {
                    ...
                    s.clone()
                } else if let Some(n) = parse_paren_name(&pkt.op) { n.to_string() } else { bail!("sect needs @<name> or (name)"); };
...
fn parse_key_name(op: &str) -> Option<&str> {
    if op.starts_with("key(") && op.ends_with(')') { return op.get(4..op.len() - 1); }
    None
}

fn parse_paren_name(op: &str) -> Option<&str> {
    let start = op.find('(')?; let end = op.rfind(')')?; if end <= start + 1 { return None; } op.get(start + 1..end)
}
