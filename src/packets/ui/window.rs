...
#[derive(Clone, Debug)]
enum Location { Top, Bottom, Left, Right, Center }

#[derive(Clone, Debug)]
enum LayoutBehavior { Flex, Grid { columns: u32, rows: u32 } }

#[derive(Clone, Debug)]
enum UiNode {
    Frame { id: String, label: Option<String>, layout: LayoutAttrs, children: Vec<UiNode> },
    Label { text: String },
    Button { label: String, action: Option<String> },
    TextEdit { var: String },
    Popup { title: String, children: Vec<UiNode> },
}
...
pub fn handle(rt: &mut Runtime, p: &Packet) -> Result<Value> {
    ...
    let body = p.body.as_ref().ok_or_else(|| anyhow::anyhow!("ui:window requires a body"))?;
...
fn parse_body(_rt: &Runtime, body: &Vec<Node>) -> Result<(Vec<UiNode>, Vec<(Vec<String>, LayoutAttrs)>)> {
                ...
                match (pkt.ns.as_deref(), pkt.op.as_str()) {
                    ...
                    (Some("frame"), id) => {
                        ...
                        let children = pkt
                            ...
                            .transpose()? 
                            .unwrap_or_default();
                        nodes.push(UiNode::Frame { id: id.to_string(), label, layout: LayoutAttrs::default(), children });
                    }
                    // [label@"text"]
                    (None, "label") => {
                        let text = pkt.arg.as_ref().and_then(|a| match a { Arg::Str(s) => Some(s.clone()), Arg::Ident(s) => Some(s.clone()), _ => None }).unwrap_or_default();
                        nodes.push(UiNode::Label { text });
                    }
                    // [separator]
                    (None, "separator") => {
                        nodes.push(UiNode::Label { text: "---".to_string() }); // placeholder for console tree
                    }
                    // [spacer@px]
                    (None, "spacer") => {
                        // store as a no-op in console tree
                        nodes.push(UiNode::Label { text: String::from("") });
                    }
                    ...
                    (Some("checkbox"), var) => {
                        ...
                        nodes.push(UiNode::Label { text: format!("[ ] {}", label.unwrap_or_else(|| var.to_string())) });
                    }
                    // [popup@"Title"]{ ... }
                    (None, "popup") => {
                        let title = pkt.arg.as_ref().and_then(|a| match a { Arg::Str(s) => Some(s.clone()), Arg::Ident(s) => Some(s.clone()), _ => None }).unwrap_or_else(|| "Popup".to_string());
                        let children = pkt
                            .body
                            .as_ref()
                            .map(|b| parse_children(_rt, b))
...
fn parse_layout_attrs(op: &str) -> LayoutAttrs {
    ...
    for part in inner.split(',') {
        ...
        match k {
            ...
            "order" => {
                if let Ok(n) = v.parse::<u32>() { out.order = Some(n); }
            }
            "behavior" => {
                let vv = v.trim();
                if vv.starts_with("grid") {
                    ...
                    if let Some(args) = vv.strip_prefix("grid").and_then(|s| s.strip_prefix('(')).and_then(|s| s.strip_suffix(')')) {
                        ...
                        out.behavior = Some(LayoutBehavior::Grid { columns: cols, rows });
                    }
                } else {
...
fn apply_layout(nodes: &mut [UiNode], target: &str, attrs: &LayoutAttrs) {
    for n in nodes.iter_mut() {
        match n {
            UiNode::Frame { id, layout, children, .. } => {
                if id == target {
                    if attrs.direction.is_some() { layout.direction = attrs.direction.clone(); }
                    if attrs.order.is_some() { layout.order = attrs.order; }
                }
...
#[cfg(feature = "ui_egui")]
fn render_gui(rt: &mut Runtime, title: &str, nodes: &mut [UiNode], _root: &LayoutAttrs) -> Result<()> {
    use eframe::{egui, NativeOptions};
    use std::sync::{Arc, Mutex};

    #[derive(Default)]
    struct ActionOut { call: Option<String> }
        ...
        fn update(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
            ...
            if let Some(call) = self.action.lock().unwrap().call.take() {
                unsafe {
                    ...
                    let node = Node::Packet(Packet { ns: None, op: "call".to_string(), arg: Some(Arg::Str(call)), body: None });
                    let _ = rt.eval(&node);
                }
    ...
    fn render_nodes(ui: &mut egui::Ui, nodes: &mut [UiNode], action: &Arc<Mutex<ActionOut>>, rt_ptr: *mut Runtime) {
        ...
        let mut render_frame = |ui: &mut egui::Ui, node: &mut UiNode| {
            if let UiNode::Frame { label, children, layout, .. } = node {
                if let Some(lbl) = label {
                    ui.heading(lbl.as_str());
                }
                match layout.behavior {
                    Some(LayoutBehavior::Grid { columns, .. }) => {
                        let grid = egui::Grid::new(format!("grid_{:p}", &*children as *const _)).num_columns(columns as usize);
                        grid.show(ui, |ui| {
                    ...
                    _ => {
                        match layout.direction {
                            Some(Direction::Horizontal) => {
                                ui.horizontal(|ui| {
                                    for ch in children.iter_mut() { render_leaf(ui, ch, action, rt_ptr); }
                                });
                            }
                            _ => {
                                for ch in children.iter_mut() { render_leaf(ui, ch, action, rt_ptr); }
        ...
        // Top region
        for n in &mut top { render_frame(ui, *n); }
        if !top.is_empty() { ui.separator(); }

        // Middle region with left | center | right
        ui.horizontal(|ui| {
            if !left.is_empty() {
                ui.vertical(|ui| { for n in &mut left { render_frame(ui, *n); } });
            }
            ui.vertical(|ui| { for n in &mut center { render_frame(ui, *n); } });
            if !right.is_empty() {
                ui.vertical(|ui| { for n in &mut right { render_frame(ui, *n); } });
            }
        });

        if !bottom.is_empty() { ui.separator(); }
        for n in &mut bottom { render_frame(ui, *n); }

        // Any non-frame nodes render at the end
        for n in others { render_leaf(ui, n, action, rt_ptr); }
    }

    fn render_leaf(ui: &mut egui::Ui, node: &mut UiNode, action: &Arc<Mutex<ActionOut>>, rt_ptr: *mut Runtime) {
        match node {
            ...
            UiNode::TextEdit { var } => {
                ...
                unsafe {
                    ...
                    if let Some(Value::Str(s)) = rt.get_var(var) { buf = s; }
                }
    ...
    let app = WindowApp { nodes: nodes.to_vec(), action: action.clone(), rt_ptr: rt as *mut Runtime, _phantom: Default::default(), title: title.to_string() };
...
fn print_tree(nodes: &[UiNode], depth: usize) {
    let pad = "  ".repeat(depth);
        for n in nodes {
            match n {
                UiNode::Frame { id, label, children, .. } => {
